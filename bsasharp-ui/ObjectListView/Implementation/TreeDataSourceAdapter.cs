...
namespace BrightIdeasSoftware
{
    ...
    public class TreeDataSourceAdapter : DataSourceAdapter
    {
        ...
        private object CalculateParent(object model)
        {
            ...
            return parentValue == null ? null : (from object x in CurrencyManager.List let key = GetKeyValue(x) where Equals(parentValue, key) select x).FirstOrDefault();
        }

        private object GetKeyValue(object model)
        {
            return DynamicHelp.Get(model, keyAspectName);
        }

        private object GetParentValue(object model)
        {
            return DynamicHelp.Get(model, parentKeyAspectName);
        }
